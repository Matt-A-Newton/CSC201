        ...
        do {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                if (i == 0) {
                    ...
                    rollScore += lastPigScore;
                } else {
                    rollScore += currentPigScore;
                }
            }
            System.out.printf("Rolling... %s & %s \n", lastPig, currentPig);

            // if the two pigs are the same type of sider, this roll score is 1
            // if the two pigs are the same, the roll score is doubled
            if (lastPig.equals(currentPig)) {
                switch(lastPig) {
                    case NO_DOT_NAME:
                    case DOT_NAME:
                        rollScore = 1;
                    ...
                    case RAZORBACK_NAME:
                    case SNOUTER_NAME:
                    case LEANING_JOWLER_NAME:
                        rollScore *= 2;
                        break;
                }

            }
            // determines if a "pig out" occured
            else if (lastPig.equals(NO_DOT_NAME) && currentPig.equals(DOT_NAME) ||
                        (lastPig.equals(DOT_NAME) && currentPig.equals(NO_DOT_NAME))) {
